  ...
  ({ className, ...props }, ref) => {
    ...
    return (
      <motion.div
        ...
        className="shadow-[inset_0_2px_4px_rgba(0,0,0,0.1)] relative flex h-9 items-center justify-center rounded-full bg-muted"
      >
        {!completed && (
          <motion.div style={{ width: adjustedWidth }} className="absolute inset-y-0 left-0 z-0 rounded-full bg-accent" />
        )}
        <AnimatePresence>
          {!completed && (
            <motion.div
            ...
            >
              <Button
                ...
                {...props}
                size="icon"
                className={cn("shadow-md rounded-full drop-shadow-xl", isDragging && "scale-105 transition-transform", className)}
              >
        ...
        <AnimatePresence>
          {completed && (
            <motion.div className="absolute inset-0 flex items-center justify-center" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              <Button ref={ref} disabled={status === "loading"} {...props} className={cn("size-full rounded-full transition-all duration-300", className)}>
