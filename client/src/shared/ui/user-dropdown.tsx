...
export const UserDropdown = ({
  ...
  const renderMenuItem = (item: MenuItemConfig, index: number) => (
    ...
    >
      ...
      {item.showAvatar && (
        <Avatar className="cursor-pointer size-6 shadow border border-border">
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent className="no-scrollbar w-[310px] rounded-2xl bg-muted p-0" align="end">
        <section className="bg-card backdrop-blur-lg rounded-2xl p-1 shadow border border-border">
          ...
          <DropdownMenuGroup>
            <DropdownMenuSub>
              ...
              <DropdownMenuPortal>
                <DropdownMenuSubContent className="bg-card backdrop-blur-lg">
          ...
          <DropdownMenuSeparator />
          <DropdownMenuGroup>
            {(MENU_ITEMS.profile as MenuItemConfig[]).map(renderMenuItem)}
          </DropdownMenuGroup>

          <DropdownMenuSeparator />
          <DropdownMenuGroup>
            {(MENU_ITEMS.premium as MenuItemConfig[]).map(renderMenuItem)}
          </DropdownMenuGroup>

          <DropdownMenuSeparator />
          <DropdownMenuGroup>
            {(MENU_ITEMS.support as MenuItemConfig[]).map(renderMenuItem)}
          </DropdownMenuGroup>
        </section>

        <section className="mt-1 p-1 rounded-2xl">
          <DropdownMenuGroup>
            {(MENU_ITEMS.account as MenuItemConfig[]).map(renderMenuItem)}
          </DropdownMenuGroup>
        </section>
      </DropdownMenuContent>
    </DropdownMenu>
