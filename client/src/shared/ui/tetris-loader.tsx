...
const TETRIS_PIECES = [
  ...
  { shape: [[1, 1], [1, 1]], color: 'bg-foreground' },
  // T-piece
  { shape: [[0, 1, 0], [1, 1, 1]], color: 'bg-foreground' },
  // L-piece
  { shape: [[1, 0], [1, 0], [1, 1]], color: 'bg-foreground' },
  // S-piece
  { shape: [[0, 1, 1], [1, 1, 0]], color: 'bg-foreground' },
  // Z-piece
...
export default function TetrisLoading({
      ...
      if (linesToClear.length > 0) {
        ...
        const newGrid = prevGrid.map((row, rowIndex) => {
          if (linesToClear.includes(rowIndex)) {
            return row.map(cell => ({ ...cell, color: 'bg-foreground animate-pulse opacity-50' }))
          }
          return row
        })
  ...
  return (
    <div>
      <div className="mb-6">
        <div className={`border-2 border-foreground bg-card ${config.padding} transition-colors`}>
          {renderGrid()}
        </div>
      </div>
