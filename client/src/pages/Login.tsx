    ...
    <div data-theme="flowdesk" className="min-h-screen flex" style={{ backgroundColor: 'var(--bg-base)' }}>
      ...
      <div className="flex-1 flex items-center justify-center p-6">
        <div className="w-full max-w-[400px]">
          ...
          <div className="mt-8 p-4 rounded-lg" style={{ background: 'var(--bg-muted)', border: '1px solid var(--border-default)' }}>
            <p className="text-[10px] font-semibold uppercase tracking-wider mb-3" style={{ color: 'var(--text-muted)' }}>Demo Accounts</p>
            <div className="space-y-2">
              {DEMO_CREDENTIALS.map((cred) => (
                <button key={cred.email} onClick={() => quickLogin(cred)} disabled={loading} className="cmd-card w-full flex items-center justify-between px-3 py-2.5 text-left group" style={{ cursor: 'pointer', opacity: loading ? 0.5 : 1 }}>
                  <div>
                    <p className="text-sm font-medium" style={{ color: 'var(--text-primary)' }}>{cred.role}</p>
                    <p className="text-xs font-mono" style={{ color: 'var(--text-muted)' }}>{cred.email}</p>
                  </div>
                  <ArrowRight className="w-4 h-4" style={{ color: 'var(--text-muted)' }} />
                </button>
              ))}
            </div>
            <p className="text-xs mt-3 text-center" style={{ color: 'var(--text-muted)' }}>Password: <span className="font-mono">demo123</span></p>
          </div>
        </div>
      </div>
    </div>
